...
import math
import copy
from numba import njit, float32, boolean, typeof
...
toto = 'flat'
spec = [('a', float32), ('probability', boolean), ('bias', typeof(toto)), ('eps', float32)]
...
class basic_metricor():
    ...
    def metric_new(self, label, score, preds, plot_ROC=False, alpha=0.2):
        ...
        PointF1PA = metrics.f1_score(label, adjust_preds, zero_division=0)
    ...
    def metric_PointF1PA(self, label, score, preds=None):
        ...
        if preds is None:
            ...
            adjust_preds = self._adjust_predicts(score, label, pred=preds)
            PointF1PA1 = metrics.f1_score(label, adjust_preds, zero_division=0)
